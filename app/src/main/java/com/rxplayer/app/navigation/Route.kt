...
sealed class Route(val route: String) {
    ...
    object Player : Route("player/{videoPath}?autoFullscreen={autoFullscreen}&playbackMode={playbackMode}&folderPath={folderPath}&playlistId={playlistId}") {
        fun createRoute(videoPath: String, autoFullscreen: Int = 0, playbackMode: Int = 0, folderPath: String = "", playlistId: Long = 0) =
            "player/${Base64.encodeToString(videoPath.toByteArray(), Base64.URL_SAFE or Base64.NO_WRAP)}?autoFullscreen=$autoFullscreen&playbackMode=$playbackMode&folderPath=${Base64.encodeToString(folderPath.toByteArray(), Base64.URL_SAFE or Base64.NO_WRAP)}&playlistId=$playlistId"
        fun decodePath(encoded: String): String =
            String(Base64.decode(encoded, Base64.URL_SAFE))
