...
object AppModule {
    ...
    private val MIGRATION_14_15 = object : Migration(14, 15) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `streams` ADD COLUMN `resolution` TEXT NOT NULL DEFAULT ''")
            db.execSQL("ALTER TABLE `streams` ADD COLUMN `codec` TEXT NOT NULL DEFAULT ''")
            db.execSQL("ALTER TABLE `streams` ADD COLUMN `frameRate` TEXT NOT NULL DEFAULT ''")
            db.execSQL("ALTER TABLE `streams` ADD COLUMN `durationMs` INTEGER NOT NULL DEFAULT 0")
        }
    }

    private val MIGRATION_15_16 = object : Migration(15, 16) {
        override fun migrate(db: SupportSQLiteDatabase) {
            db.execSQL("ALTER TABLE `folders` ADD COLUMN `privacyMask` INTEGER NOT NULL DEFAULT 0")
            db.execSQL("ALTER TABLE `playlists` ADD COLUMN `privacyMask` INTEGER NOT NULL DEFAULT 0")
        }
    }
